...
class StateMachine:
    ...
    def set_current_phase(self, phase: int) -> int:
        """Force the FSM's current phase to a specific value from the plan (for external managers)."""
        try:
            p = int(phase)
        except Exception:
            return self._current_phase
        if p in self._performance_plan:
            self._current_phase_index = self._performance_plan.index(p)
            self._current_phase = p
        else:
            # If not in plan, do not change index; keep current
            logger.debug("StateMachine.set_current_phase ignored: %s not in plan %s", p, self._performance_plan)
        return self._current_phase
