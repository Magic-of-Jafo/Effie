...
async def play(audio_bytes: bytes, *, config: Optional[Dict[str, Any]] = None, started_at_monotonic: Optional[float] = None) -> None:
    ...
    if backend_pref == "elevenlabs":
        try:
            ...
            logger.info("Playback: second pass (elevenlabs)")
            await asyncio.to_thread(_play_el, audio_bytes)
            return
        except Exception as exc:
...
async def stream_and_play(text: str, *, config: Optional[Dict[str, Any]] = None) -> None:
    """Stream TTS audio from ElevenLabs SDK and play locally using elevenlabs.stream().

    Honors playback.warmup_ms, playback.settle_ms.
    """
    if not isinstance(text, str) or not text.strip():
        return
