...
async def play(audio_bytes: bytes, *, config: Optional[Dict[str, Any]] = None, started_at_monotonic: Optional[float] = None) -> None:
    ...
    - If backend is set to 'elevenlabs', use elevenlabs.play for bytes (requires mpv/ffmpeg per SDK).
    ...
    # ElevenLabs SDK backend (plays bytes via mpv/ffmpeg)
    if backend_pref == "elevenlabs":
        try:
            from elevenlabs import play as el_play  # type: ignore

            def _play_el(data: bytes) -> None:
                el_play(data)
            ...
            logger.info("Playback backend: elevenlabs.play")
            # Warmup: play a short silent WAV first
            if warmup_ms > 0:
                silence_bytes = _generate_silence_wav(warmup_ms)
                await asyncio.to_thread(_play_el, silence_bytes)
            if settle_ms > 0:
                await asyncio.sleep(max(0.0, settle_ms / 1000.0))
                logger.info("Playback: settled for %d ms", settle_ms)
            await asyncio.to_thread(_play_el, audio_bytes)
            return
        except Exception as exc:
    ...
    if backend_pref == "winsound":
        try:
            ...
            # Warmup with separate silent WAV to wake the device
            if warmup_ms > 0:
                silence_bytes = _generate_silence_wav(warmup_ms)
                await asyncio.to_thread(_play_blocking_ws, silence_bytes)
            if settle_ms > 0:
                await asyncio.sleep(max(0.0, settle_ms / 1000.0))
                logger.info("Playback: settled for %d ms", settle_ms)
    ...
    try:
        ...
        if warmup_ms > 0:
            silence_bytes = _generate_silence_wav(warmup_ms)
            await asyncio.to_thread(_play_blocking_sa, silence_bytes)
        if settle_ms > 0:
            await asyncio.sleep(max(0.0, settle_ms / 1000.0))
            logger.info("Playback: settled for %d ms", settle_ms)
...
def is_streaming_enabled(config: Optional[Dict[str, Any]] = None) -> bool:
    cfg = config or load_config()
    tts_cfg: Dict[str, Any] = cfg.get("tts", {})
    return bool(tts_cfg.get("streaming_enabled", False))


async def stream_and_play(text: str, *, config: Optional[Dict[str, Any]] = None, started_at_monotonic: Optional[float] = None) -> None:
    """Stream TTS audio from ElevenLabs SDK and play locally using elevenlabs.stream().

    Honors playback.warmup_ms, playback.settle_ms. If started_at_monotonic is provided,
    logs the elapsed time (ms) from that moment to when streaming playback is initiated.
    """
    if not isinstance(text, str) or not text.strip():
        return
    ...
    try:
        ...
        from elevenlabs import stream as el_stream, play as el_play  # type: ignore
    except Exception as exc:
    ...
    if warmup_ms > 0:
        silence_bytes = _generate_silence_wav(warmup_ms)
        await asyncio.to_thread(el_play, silence_bytes)
    if settle_ms > 0:
        await asyncio.sleep(max(0.0, settle_ms / 1000.0))
        logger.info("Playback: settled for %d ms", settle_ms)
