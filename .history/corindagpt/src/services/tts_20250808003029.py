...
async def synthesize(text: str, *, http_client: Optional[httpx.AsyncClient] = None, config: Optional[Dict[str, Any]] = None) -> bytes:
    ...
    try:
        ...
        try:
            ...
            raise
        pcm = resp.content  # raw PCM16LE at 16kHz mono
        if not pcm:
        ...
        logger.info("TTS: synthesized %d bytes (WAV)", len(wav_bytes))
        return wav_bytes
    finally:
        if own_client:
            await client.aclose()
...
async def play(audio_bytes: bytes) -> None:
    ...
    if not audio_bytes:
        return
    try:
        import simpleaudio  # lazy import
    ...
    await asyncio.to_thread(_play_blocking, audio_bytes)
