...
async def synthesize(
    ...
    *,
    ...
    payload: Dict[str, Any] = {
        "text": text.strip(),
        "model_id": model_id,
        # For non-streaming we request a common MP3 profile
        "output_format": "mp3_44100_128",
        "voice_settings": ev_cfg.get("voice_settings") or {"stability": 0.3, "similarity_boost": 0.7},
    }

    async def _request_streaming() -> bytes:
    ...
    async def _request_non_streaming() -> bytes:
    ...
    try:
        ...
        content_type = ""
        try:
            content_type = (resp.headers.get("Content-Type") or "").lower()  # type: ignore[name-defined]
        except Exception:
            pass
