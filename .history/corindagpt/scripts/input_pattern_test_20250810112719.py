...
from corindagpt.src.services.audio_queue import build_default_queue
...
async def run() -> None:
    ...
    # Build audio queue from corindagpt/assets/sfx (created if missing)
    queue = build_default_queue(cfg)
    sfx_dir = queue._items[0].parent if not queue.is_empty() else pathlib.Path(__file__).resolve().parents[1] / "assets" / "sfx"
    ...
    print("Actions:")
    print(" - BRIEF: quick tap (plays next MP3 in queue)")
    ...
    async def on_event(evt: InputEvent) -> None:
        ...
        if getattr(evt, "pattern", None) == InputPattern.BRIEF:
            played = await queue.play_next(config=cfg)
            if played is not None:
                print(f"Played: {played.name}")
            else:
                print("Queue empty. Drop MP3s into corindagpt/assets/sfx and retry.")
